...
public class PlayerControl : MonoBehaviour
{
    ...
    private void FollowBea()
    {
        ...
        if (Physics.Raycast(isGroundedTranform.position, isGroundedTranform.TransformDirection(-Vector3.up), out hit, 0.5f))
        {
            isGrounded = true;
        }
    ...
    private void HandleAttack(float delta)
    {
        ...
        if(timerAttack >= 1f && timerAttack <= 1.5f)
        {
            isAttacking = true;
        }
